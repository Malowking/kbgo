...
func SaveFileToRustFS(ctx context.Context, client *minio.Client, bucketName string, knowledgeId string, fileName string, file io.ReadSeeker) (localPath string, rustfsKey string, err error) {
	...
	if err != nil {
		...
		return localPath, "", fmt.Errorf("failed to open local file for upload: %w", err)
	}
	...
	if err != nil {
		...
		return localPath, "", fmt.Errorf("failed to get file stat: %w", err)
	}
	fileSize := stat.Size()
	...
	if err != nil && err != io.EOF {
		...
		return localPath, "", fmt.Errorf("failed to read file header: %w", err)
	}
	...
	if err != nil {
		...
		return localPath, "", fmt.Errorf("failed to seek file to beginning: %w", err)
	}
	...
	if err != nil {
		...
		return localPath, "", fmt.Errorf("failed to upload to RustFS: %w", err)
	}
